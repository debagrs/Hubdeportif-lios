...
const DEFAULT_THEME = {
  ...
  contentWidth: 1280, gridColumns: 2, gridGap: 24, sectionGap: 110, sidePadding: 28, projectLayout: 'grid', projectInfo: 'below', textAlign: 'left',
...
const ALLOWED_FONTS = new Set(['Lexend','Space Grotesk','DM Sans','Manrope','Archivo','IBM Plex Sans','Syne','Bricolage Grotesque','Playfair Display','Cormorant Garamond','Lora','Bebas Neue','JetBrains Mono','IBM Plex Mono']);
...
const sanitizeTheme = (value: unknown) => {
  ...
  return {
    ...
    headingFont: ALLOWED_FONTS.has(text(raw.headingFont)) ? text(raw.headingFont) : DEFAULT_THEME.headingFont,
    bodyFont: ALLOWED_FONTS.has(text(raw.bodyFont)) ? text(raw.bodyFont) : DEFAULT_THEME.bodyFont,
    monoFont: ALLOWED_FONTS.has(text(raw.monoFont)) ? text(raw.monoFont) : DEFAULT_THEME.monoFont,
    headingWeight: safeNumber(raw.headingWeight,300,900,DEFAULT_THEME.headingWeight,100), bodyWeight: safeNumber(raw.bodyWeight,300,800,DEFAULT_THEME.bodyWeight,100),
    headingCase: safeEnum(raw.headingCase,['none','uppercase','lowercase'] as const,DEFAULT_THEME.headingCase), headingStyle: safeEnum(raw.headingStyle,['normal','italic'] as const,DEFAULT_THEME.headingStyle), headingTracking: safeNumber(raw.headingTracking,-8,12,DEFAULT_THEME.headingTracking,1), bodySize: safeNumber(raw.bodySize,14,22,DEFAULT_THEME.bodySize,1), bodyLeading: safeNumber(raw.bodyLeading,1.2,2,DEFAULT_THEME.bodyLeading,.05), headingScale: safeEnum(raw.headingScale,['compact','balanced','oversized'] as const,DEFAULT_THEME.headingScale),
    contentWidth: Number(safeEnum(String(raw.contentWidth),['900','1100','1280','1440'] as const,String(DEFAULT_THEME.contentWidth))),
    gridColumns: safeNumber(raw.gridColumns,1,4,DEFAULT_THEME.gridColumns,1), gridGap: safeNumber(raw.gridGap,0,64,DEFAULT_THEME.gridGap,1), sectionGap: safeNumber(raw.sectionGap,40,200,DEFAULT_THEME.sectionGap,1), sidePadding: safeNumber(raw.sidePadding,12,72,DEFAULT_THEME.sidePadding,1),
    projectLayout: safeEnum(raw.projectLayout,['grid','editorial','stack'] as const,DEFAULT_THEME.projectLayout), projectInfo: safeEnum(raw.projectInfo,['below','overlay','split'] as const,DEFAULT_THEME.projectInfo), textAlign: safeEnum(raw.textAlign,['left','center'] as const,DEFAULT_THEME.textAlign),
    cardRadius: safeNumber(typeof raw.cardRadius === 'number' ? raw.cardRadius : radiusFallback,0,64,DEFAULT_THEME.cardRadius,1), buttonRadius: safeNumber(raw.buttonRadius,0,999,DEFAULT_THEME.buttonRadius,1), borderWidth: safeNumber(raw.borderWidth,0,4,DEFAULT_THEME.borderWidth,1), cardStyle: safeEnum(raw.cardStyle,['surface','outline','minimal'] as const,DEFAULT_THEME.cardStyle), shadow: safeEnum(raw.shadow,['none','soft','hard'] as const,DEFAULT_THEME.shadow), buttonStyle: safeEnum(raw.buttonStyle,['solid','outline','text'] as const,DEFAULT_THEME.buttonStyle),
    projectAspect: safeEnum(raw.projectAspect,['landscape','square','portrait','wide','auto'] as const,DEFAULT_THEME.projectAspect), imageFit: safeEnum(raw.imageFit,['cover','contain'] as const,DEFAULT_THEME.imageFit), heroStyle: safeEnum(raw.heroStyle,['split','full','text'] as const,DEFAULT_THEME.heroStyle), heroHeight: safeEnum(raw.heroHeight,['compact','medium','tall'] as const,DEFAULT_THEME.heroHeight), navStyle: safeEnum(raw.navStyle,['minimal','floating','bar'] as const,DEFAULT_THEME.navStyle), graphicStyle: safeEnum(raw.graphicStyle,['none','gradient','grid','dots','geometric'] as const,DEFAULT_THEME.graphicStyle), animation: safeEnum(raw.animation,['none','subtle','dynamic'] as const,DEFAULT_THEME.animation), iconStyle: safeEnum(raw.iconStyle,['outline','bold','boxed','none'] as const,DEFAULT_THEME.iconStyle), iconStroke: safeNumber(raw.iconStroke,1,3,DEFAULT_THEME.iconStroke,.2),
