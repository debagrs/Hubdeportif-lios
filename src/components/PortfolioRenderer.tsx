...
function MediaImage({src,alt,className,loading}:{src:string;alt:string;className?:string;loading?:'eager'|'lazy'}){
  const [failed,setFailed]=React.useState(false);
  React.useEffect(()=>setFailed(false),[src]);
  if(!src||failed)return <div className={`pf-media-fallback ${className||''}`} role="img" aria-label={`${alt} — imagem indisponível`}><span>imagem indisponível</span></div>;
  return <img src={src} alt={alt} className={className} loading={loading} decoding="async" referrerPolicy="no-referrer" onError={()=>setFailed(true)}/>;
}
...
export default function PortfolioRenderer({ portfolio, embedded=false }: Props) {
  ...
  function ProjectCopy({project}:{project:Project}){
    return <div className="pf-project-copy">
      ...
      {project.gallery?.length>0&&<div className="pf-gallery">{project.gallery.map((url,i)=><MediaImage src={url} alt={`Imagem ${i+1} do projeto ${project.title}`} key={`${url}-${i}`} loading="lazy"/>)}</div>}
  ...
  const renderProject=(project:Project,index:number)=><article className={`pf-project ${cardClass} ${infoClass} ${project.featured&&theme.featuredSpan?'pf-project-featured':''}`} key={project.id} style={{animationDelay:`${Math.min(index,8)*70}ms`}}><div className="pf-project-media">{project.coverUrl?<MediaImage src={project.coverUrl} alt={`Capa do projeto ${project.title}`} loading="lazy"/>:<div className="pf-project-placeholder"/>}{theme.projectInfo==='overlay'&&<div className="pf-overlay-shade"/>}{theme.projectInfo==='overlay'&&<ProjectCopy project={project}/>}</div>{theme.projectInfo!=='overlay'&&<ProjectCopy project={project}/>} {theme.projectInfo==='overlay'&&project.gallery?.length>0&&<div className="pf-gallery pf-gallery-after-overlay">{project.gallery.map((url,i)=><MediaImage src={url} alt={`Imagem ${i+1} do projeto ${project.title}`} key={`overlay-${url}-${i}`} loading="lazy"/>)}</div>}</article>;

  const AboutSection=()=> <section id="about" className="pf-section"><div className="pf-container"><div className="pf-section-heading"><span>{theme.iconStyle!=='none'&&glyph('about',16)} {theme.aboutLabel}</span><h2>{theme.aboutLabel}</h2></div><div className="pf-about-grid"><div>{theme.showAvatar&&<div className="pf-avatar">{portfolio.avatarUrl?<MediaImage src={portfolio.avatarUrl} alt={`Retrato de ${portfolio.displayName}`}/>:portfolio.displayName.slice(0,2).toUpperCase()}</div>}</div><div><p className="pf-about-text">{portfolio.about||portfolio.bio}</p></div></div></div></section>;
  ...
  return <div className={`portfolio-site ${graphicClass} ${animationClass} ${navClass} ${iconClass} ${heroScaleClass} ${aspectClass} ${alignClass}`} style={style}>
    ...
    <header id="top" className={`pf-hero pf-hero-${theme.heroStyle}`}><div className="pf-container pf-hero-inner"><div className="pf-hero-copy"><span className="pf-eyebrow">{theme.heroEyebrow}</span><h1>{portfolio.displayName}</h1><p className="pf-headline">{portfolio.headline}</p>{theme.showBioInHero&&portfolio.bio&&<p className="pf-hero-bio">{portfolio.bio}</p>}<a className={`pf-button ${buttonClass}`} href="#projects">{theme.projectsLabel}{theme.iconStyle!=='none'&&glyph('down')}</a></div>{theme.heroStyle!=='text'&&<div className="pf-hero-media">{portfolio.heroImageUrl?<MediaImage src={portfolio.heroImageUrl} alt={`Imagem de abertura do portfólio de ${portfolio.displayName}`} loading="eager"/>:<div className="pf-hero-placeholder"><span style={{background:theme.accent}}/><span style={{background:theme.secondary}}/></div>}</div>}</div></header>
