import type { PortfolioDetail, PortfolioSummary, Project, SessionUser, StudentAccount, StudentInvite } from '../types';
...
export const hubApi = {
  ...
  async uploadImage(file: File, kind: 'avatar' | 'hero' | 'cover' | 'gallery', portfolioId?: string) {
    ...
    if (!upload.ok || !result?.secure_url) throw new Error(result?.error?.message || 'O Cloudinary não conseguiu receber a imagem.');
