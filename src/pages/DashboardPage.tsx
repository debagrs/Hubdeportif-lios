...
import {
  CheckCircle2, CircleAlert, ExternalLink, Eye, ImagePlus, KeyRound, LayoutDashboard,
  LogOut, Palette, Plus, Save, Send, Settings2, Trash2, UserPlus, UsersRound,
} from 'lucide-react';
...
const emptyProject = (portfolioId = ''): Project => ({
  id:'', portfolioId, slug:'', title:'', subtitle:'', category:'Projeto de Design', year:String(new Date().getFullYear()), summary:'', body:'', coverUrl:'', gallery:[], externalUrl:'', tags:[], featured:false, published:true, sortOrder:0,
});
...
export default function DashboardPage() {
  ...
  useEffect(()=>{
    ...
    hubApi.session().then(async result=>{
      ...
      if(result.user.role==='admin'){
        const [s,i]=await Promise.all([hubApi.listStudents(),hubApi.listInvites()]);
        setStudents(s.students);setInvites(i.invites);
        if(s.students[0]){setSelectedStudentId(s.students[0].id);await loadPortfolio(s.students[0].id);} else setTab('students');
  ...
  async function deleteStudent(student:StudentAccount){if(!window.confirm(`Excluir a conta e o portfólio de ${student.displayName}?`))return;setBusy(true);clearNotice();try{await hubApi.deleteStudent(student.id);const s=await hubApi.listStudents();setStudents(s.students);setMessage('Conta excluída.');if(selectedStudentId===student.id){if(s.students[0]){setSelectedStudentId(s.students[0].id);await loadPortfolio(s.students[0].id);}else{setSelectedStudentId(undefined);setPortfolio(null);setProjects([]);}}}catch(e){setError(e instanceof Error?e.message:'Não foi possível excluir.');}finally{setBusy(false);}}
  ...
  async function logout(){await hubApi.logout().catch(()=>undefined);navigate('/entrar');}
  ...
  return <div className="lab-page"><HubHeader compact/><main className="lab-main"><section className="lab-section"><div className="lab-container">
    <div className="section-head"><div><p className="lab-kicker">PAINEL • LABINTERFACE</p><h2>{title}</h2><p className="dashboard-help">O painel pertence ao Hub. O site público do estudante, porém, é independente e pode assumir outro design system por completo.</p></div><div className="dashboard-actions" style={{marginTop:0}}>{portfolio?.slug&&<><a className="lab-secondary" href={`/portfolio/${portfolio.slug}`} target="_blank" rel="noreferrer"><ExternalLink size={16}/> Ver site</a><a className="lab-secondary" href={`/embed/${portfolio.slug}`} target="_blank" rel="noreferrer"><Eye size={16}/> Ver versão embed</a></>}<button className="lab-secondary" onClick={logout}><LogOut size={16}/> Sair</button></div></div>
    ...
    {isAdmin&&students.length>0&&<div className="dashboard-card"><p className="lab-label" style={{color:'#D2A979',marginBottom:12}}>PORTFÓLIO EM EDIÇÃO</p><div className="dashboard-actions" style={{marginTop:0}}>{students.map(student=><button key={student.id} className="lab-secondary" style={selectedStudentId===student.id?{borderColor:'#00FFFF',color:'#00FFFF'}:{}} onClick={()=>chooseStudent(student)}>{student.displayName}</button>)}</div></div>}

    <div className="dashboard-layout"><aside className="dashboard-side" aria-label="Seções do painel">
      <button className={tab==='portfolio'?'active':''} onClick={()=>setTab('portfolio')}><LayoutDashboard size={16}/> Conteúdo</button>
      <button className={tab==='design'?'active':''} onClick={()=>setTab('design')}><Palette size={16}/> Design system</button>
      ...
      {tab==='design'&&portfolio&&<><DesignSystemEditor portfolio={portfolio} onChange={setPortfolio}/><div className="dashboard-savebar sticky"><button className="lab-primary" disabled={busy} onClick={()=>savePortfolio()}><Save size={17}/>{busy?'Salvando…':'Salvar design system'}</button>{portfolio.slug&&<a className="lab-secondary" href={`/portfolio/${portfolio.slug}`} target="_blank" rel="noreferrer"><ExternalLink size={16}/> Abrir resultado</a>}</div></>}
      ...
      {tab==='projects'&&portfolio&&<>
        ...
        {projectDraft&&<form className="dashboard-card" onSubmit={saveProject}><p className="lab-label" style={{color:'#D2A979'}}>{projectDraft.id?'EDITAR PROJETO':'NOVO PROJETO'}</p><h2>{projectDraft.id?'Editar projeto':'Novo projeto'}</h2><div className="lab-form-grid">
          ...
          <div className="lab-field"><label>Link externo</label><input type="url" value={projectDraft.externalUrl||''} onChange={e=>setProjectDraft({...projectDraft,externalUrl:e.target.value})}/></div>
