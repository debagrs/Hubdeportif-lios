...
import type { PortfolioDetail, Project, SessionUser, StudentAccount } from '../types';
...
export default function DashboardPage() {
  ...
  const [newStudent, setNewStudent] = useState({ email: '', displayName: '', slug: '' });
  ...
  async function refreshStudents() {
    ...
    const result = await hubApi.listStudents();
  ...
  async function createStudent(event: FormEvent) {
    ...
    setCreatedPassword('');
    try {
      const result = await hubApi.createStudent(newStudent);
      setCreatedPassword(result.temporaryPassword);
      const s = await hubApi.listStudents();
      setStudents(s.students);
      setNewStudent({ email: '', displayName: '', slug: '' });
      setMessage(result.emailSent
        ? 'Conta criada e e-mail de validação enviado.'
        : 'Conta criada. Copie a senha temporária abaixo; o envio por e-mail ainda não está configurado.');
      if (!selectedStudentId && s.students[0]) {
        setSelectedStudentId(s.students[0].id);
        await loadPortfolio(s.students[0].id);
      }
    } catch (e) {
              ...
              <div className="dashboard-panel">
                {tab === 'portfolio' && portfolio && (
                  <form onSubmit={savePortfolio}>
                    ...
                    <div className="dashboard-card">
                      <p className="lab-label" style={{ color: '#F48A79' }}>UPLOAD DIRETO • R2</p>
                      <h2>Imagens</h2>
                ...
                {tab === 'students' && isAdmin && (
                  <>
                    <form className="dashboard-card" onSubmit={createStudent}>
                      <p className="lab-label" style={{ color: '#86EFAC' }}>ADMINISTRAÇÃO</p>
                      <h2>Novo estudante</h2>
                      <p className="dashboard-help">O Hub aceita até cinco contas. Não existe cadastro público: você cria os acessos e cada estudante só edita o próprio portfólio.</p>
                      <div className="lab-form-grid">
                        <div className="lab-field"><label>Nome</label><input required value={newStudent.displayName} onChange={e => setNewStudent({ ...newStudent, displayName: e.target.value })} /></div>
                        <div className="lab-field"><label>E-mail institucional/pessoal</label><input required type="email" value={newStudent.email} onChange={e => setNewStudent({ ...newStudent, email: e.target.value })} /></div>
                        <div className="lab-field"><label>Endereço do portfólio</label><input required placeholder="nome-sobrenome" value={newStudent.slug} onChange={e => setNewStudent({ ...newStudent, slug: e.target.value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-') })} /></div>
                      </div>
                      <div className="dashboard-actions"><button className="lab-primary" disabled={busy || students.length >= 5}><UserPlus size={17} /> Criar estudante</button></div>
                      {students.length >= 5 && <div className="lab-message" style={{ marginTop: 16 }}>Os cinco espaços de estudante já foram criados.</div>}
                      {createdPassword && <div className="lab-message success" style={{ marginTop: 16 }}><strong>Senha temporária:</strong> {createdPassword}<br />Guarde-a até confirmar que o estudante recebeu o convite.</div>}
                    </form>

                    <div className="dashboard-card">
                      <p className="lab-label" style={{ color: '#D2A979' }}>05 ESPAÇOS</p>
                      <h2>Estudantes</h2>
                      <div className="dashboard-list">
                        {students.length === 0 && <div className="lab-empty">Ainda não há estudantes cadastrados.</div>}
                        {students.map(student => (
                          <div className="dashboard-row" key={student.id}>
                            ...
                            <div className="dashboard-actions" style={{ marginTop: 0 }}>
                              {student.emailVerified
                                ? <span className="dashboard-badge"><CheckCircle2 size={13} /> e-mail validado</span>
                                : <button className="lab-secondary" onClick={() => hubApi.resendVerification(student.id).then(() => setMessage('E-mail de validação reenviado.')).catch(e => setError(e.message))}><Send size={14} /> reenviar validação</button>}
                ...
                {tab === 'system' && isAdmin && (
                  <div className="dashboard-card">
                    ...
                    <div className="integration-grid">
                      ...
                      <IntegrationStatus name="Cloudflare R2" description="imagens e galerias" ready={Boolean(health?.storageConfigured)} />
